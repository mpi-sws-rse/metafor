...
This generates a file "learned_model_serverk.pkl" insider "models/" for       
each server k=1..n . The pkl file stores the all the variables associated     
with the learned model. For instance, for k=2, the training generates two     
models, each for the corresponding server. The two files can be found at     
