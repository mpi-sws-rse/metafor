...
class StatData:
    ...
    def as_csv(self) -> str:
        return (
            f"{int(self.start_t)},{self.rho},{self.service_time},{self.run_name},{self.qlen},{self.retries},"
...
def stat_data_average(stat_data_v: List[StatData]) -> StatData:
    data: StatData = StatData(
        stat_data_v[0].start_t, stat_data_v[0].rho, stat_data_v[0].run_name
    )
...
def stat_data_variance(stat_data_v: List[StatData]) -> StatData:
    ...
    data.service_time = avg(
        [
            abs(stat_data.service_time - average_data.service_time) ** 2
            for stat_data in stat_data_v
        ]
    )
    data.qlen = avg(
        [abs(stat_data.qlen - average_data.qlen) ** 2 for stat_data in stat_data_v]
    )
    data.retries = avg(
        [
            abs(stat_data.retries - average_data.retries) ** 2
            for stat_data in stat_data_v
        ]
    )
    data.dropped = avg(
        [
            abs(stat_data.dropped - average_data.dropped) ** 2
            for stat_data in stat_data_v
        ]
    )
    data.runtime = avg(
        [
            abs(stat_data.runtime - average_data.runtime) ** 2
            for stat_data in stat_data_v
        ]
    )
    return data
