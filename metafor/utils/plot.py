...
def plot_results(step_time: int, main_queue_ave_len_seq: List[float], main_queue_var_len_seq: List[float],
                 main_queue_std_len_seq: List[float], runtime_seq: List[float], figure_name: str):
    ...
    ax1.plot(time, main_queue_ave_len_seq, color='tab:blue')
    ax1.set_xlabel('Time bound (ms)', fontsize=8)
    ax1.set_ylabel('Average queue length', fontsize=8)
    ax1.grid('on')
    ax1.set_xlim(0, max(time))

    ax2 = plt.subplot(ax[1, 0])  # row 1, col 0
    ax2.plot(time, main_queue_var_len_seq, color='tab:red')
    ax2.set_xlabel('Time bound (ms)', fontsize=8)
    ax2.set_ylabel('Variance of queue length', fontsize=8)
    ax2.grid('on')
    ax2.set_xlim(0, max(time))

    ax3 = plt.subplot(ax[2, 0])  # row 2, col 0
    ax3.plot(time, main_queue_std_len_seq, color='tab:purple')
    ax3.set_xlabel('Time bound (ms)', fontsize=8)
    ax3.set_ylabel('Standard deviation of queue length', fontsize=8)
    ax3.grid('on')
...
def plot_results_latency(input_seq: List[float], mean_seq: List[float], lower_bound_seq: List[float],
                         upper_bound_seq: List[float], x_axis: str, y_axis: str, figure_name: str, color1: str,
                         color2: str):
    plt.rc('font', size=14)
    ...
    plt.fill_between(input_seq, lower_bound_seq, upper_bound_seq, color=color2, alpha=0.4)
...
def plot_bar_data(step_size: float, input_seq: List[float], low_regime_prob_seq: List[float],
                  high_regime_prob_seq: List[float], mean_rt_seq_su, mean_rt_seq_us,
                  x_axis: str, figure_name: str, color1: str, color2: str, color3: str):
    plt.rc('font', size=14)
    plt.rcParams["figure.figsize"] = [10, 10]
    plt.rcParams["figure.autolayout"] = True
    fig, ax = plt.subplots()
    bar_width = 0.2 * step_size
    max_rt = max([min(mean_rt_seq_su[i], mean_rt_seq_us[i]) for i in range(len(input_seq))])

    for i in range(len(input_seq)):
        sum_prob = low_regime_prob_seq[i] + high_regime_prob_seq[i]
        ax.bar(input_seq[i], low_regime_prob_seq[i] / sum_prob, width=bar_width, color=color1,
               label=f'Normalized Prob for low mode' if i == 0 else "")
        ax.bar(input_seq[i], high_regime_prob_seq[i] / sum_prob, width=bar_width,
               bottom=np.array(low_regime_prob_seq[i] / sum_prob) + (
                       min(mean_rt_seq_su[i], mean_rt_seq_us[i]) / max_rt), color=color3,
               label=f'Normalized Prob for high mode' if i == 0 else "")
        ax.bar(input_seq[i], min(mean_rt_seq_su[i], mean_rt_seq_us[i]) / max_rt, width=bar_width,
               bottom=low_regime_prob_seq[i] / sum_prob, color=color2,
               alpha=0.6,
...
def plot_results_reset(input_seq: List[float], mean_seq: List[float], lower_bound_seq: List[float],
                       upper_bound_seq: List[float], x_axis: str, y_axis: str, figure_name: str,
                       color1: str, color2: str, nominal_value):
    plt.rc('font', size=14)
    plt.rcParams["figure.figsize"] = [5, 5]
    plt.rcParams["figure.autolayout"] = True
...
def trigger_plot_generator(lambda_fault, fault_start_time, fault_duration, lambda0, time_step, sim_time,
                           file_name: str):
    input_seq = []
    time_seq = []
...
def plot_heatmap(input_seq: List[List[float]], low_regime_prob_seq: List[float], high_regime_prob_seq: List[float],
                 T_mixing_seq, x_axis: str, y_axis: str, figure_name: str):
    plt.rc('font', size=14)
    plt.rcParams["figure.figsize"] = [10, 10]
    plt.rcParams["figure.autolayout"] = True
    ...
    z = np.array(
        [min(low_regime_prob_seq[i], high_regime_prob_seq[i]) * T_mixing_seq[i] for i in range(len(input_seq))])
...
def plot_Nyquist(input_seq: List[float], low_regime_prob_seq: List[float], high_regime_prob_seq: List[float],
                 T_mixing_seq, x_axis: str, y_axis: str, figure_name: str):
    plt.rc('font', size=14)
    plt.rcParams["figure.figsize"] = [10, 10]
    plt.rcParams["figure.autolayout"] = True
    x = T_mixing_seq
    y = [min(low_regime_prob_seq[i], high_regime_prob_seq[i]) for i in range(len(input_seq))]
    z = input_seq
    cmap = plt.get_cmap('viridis')
    norm = plt.Normalize(vmin=min(z), vmax=max(z))

    points = np.array([x, y]).T.reshape(-1, 1, 2)  # Reshape the x and y points for LineCollection
    ...
    ax.axvspan(2000, max(x) * 1.1, ymin=.5, ymax=1, facecolor='red', alpha=0.3, label='Metastable region')
...
def plot_quiver(input_seq: List[float], low_regime_prob_seq: List[float], high_regime_prob_seq: List[float],
                T_mixing_seq, x_axis: str, y_axis: str, figure_name: str):
    x = T_mixing_seq
    y = [min(low_regime_prob_seq[i], high_regime_prob_seq[i]) for i in range(len(input_seq))]
    z = input_seq
...
def plot_scatter(input_seq: List[float], low_regime_prob_seq: List[float], high_regime_prob_seq: List[float],
                 T_mixing_seq, x_axis: str, y_axis: str, figure_name: str):
    x = T_mixing_seq
    y = [min(low_regime_prob_seq[i], high_regime_prob_seq[i]) for i in range(len(input_seq))]
    z = input_seq
    ...
    for i in range(1, len(z), 1):
        plt.arrow(x[i - 1], y[i - 1], x[i] - x[i - 1], y[i] - y[i - 1],
...
def plot_2D(input_seq: List[float], low_regime_prob_seq: List[float], high_regime_prob_seq: List[float],
            T_mixing_seq, x_axis: str, y_axis: str, figure_name: str):
    plt.rc('font', size=14)
    plt.rcParams["figure.figsize"] = [10, 10]
    plt.rcParams["figure.autolayout"] = True
...
def plot_mixing_time(input_seq: List[float], low_regime_prob_seq: List[float], high_regime_prob_seq: List[float],
                     T_mixing_seq, x_axis: str, y_axis: str, figure_name: str, color1: str):
    plt.rc('font', size=14)
    plt.rcParams["figure.figsize"] = [5, 5]
    plt.rcParams["figure.autolayout"] = True

    plt.figure()  # row 0, col 0
    output_seq = [min(low_regime_prob_seq[i], high_regime_prob_seq[i]) * T_mixing_seq[i]
                  for i in range(len(input_seq))]
...
def plot_metastability_index(input_seq: List[float], output_seq: List[float], x_axis: str, y_axis: str,
                             figure_name: str, color: str):
    plt.rc('font', size=14)
    plt.rcParams["figure.figsize"] = [5, 5]
    plt.rcParams["figure.autolayout"] = True
