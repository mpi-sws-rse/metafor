...
class SingleServerCTMC(CTMC):
    ...
    def generator_mat_exact(self, transition_matrix: bool = False):
        ...
        for total_ind in range(state_num):
            ...
            if n_main_queue == 0:  # queue is empty
                ...
                if n_retry_queue > 0:
                    Q[
                        total_ind, self.index_composer(n_main_queue, n_retry_queue - 1)
                    ] = (n_retry_queue * mu_drop_base)
                    Q[
                        total_ind,
                        self.index_composer(n_main_queue + 1, n_retry_queue - 1),
                    ] = (
    ...
    def get_stationary_distribution(self):
        ...
        if (
            sum(self.pi) < -0.01
        ):  # the null space can return the negation of the stationary distribution
            self.pi = -self.pi
        return self.pi
    ...
    def latency_average(self, pi, job_type: int = -1) -> float:
        ...
        for n_main_queue in range(main_queue_size):
            ...
            val += (
                weight
                * (self.params.lambdaas[job_type] / self.params.lambdaa)
                * n_main_queue
                * (1 / mu0_p)
            )
        return val[0]
    ...
    def server_population_queue_model(self, Q, pi_q_seq, plot_params: PlotParameters):
        ...
        Q_T = np.transpose(Q)
        print(
            "Computing the average length of the main queue/orbit in the time interval %d-%d"
            % (step_time, sim_time)
        )
        for t in range(step_time, sim_time + 1, step_time):
            ...
            main_queue_variance_length = self.main_queue_size_var(
                pi_q_new, main_queue_mean_length
            )
            main_queue_var_len_seq.append(main_queue_variance_length)
            main_queue_std_len_seq.append(
                self.main_queue_size_std(main_queue_variance_length)
            )
            retry_queue_avg_len_seq.append(self.retry_queue_average_size(pi_q_new))
            runtime_seq.append(time.time() - start_time)

        return (
            main_queue_avg_len_seq,
            main_queue_var_len_seq,
            main_queue_std_len_seq,
            retry_queue_avg_len_seq,
            runtime_seq,
        )
    ...
    def queues_avg_len(self, Q, t: int, plot_params: PlotParameters) -> (float, float):
        ...
        main_queue_avg_len, _, _, retry_queue_avg_len, _ = (
