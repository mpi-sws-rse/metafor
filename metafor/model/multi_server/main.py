...
def main():
    ...
    parser.add_argument('-lambda_fault', nargs='+',
                        ...
                        default=[8, 4.5])
    parser.add_argument('-lambda_reset', nargs='+', help='job arrival rate after eliminating fault', default=[8, .25])
    parser.add_argument('-mu0_p', nargs='+', help='initial service rate', default=[10, 15])
    parser.add_argument('-config_list', nargs='+', help='the set of configs (currently only arrival rates)',
                        default=[[8, .5], [8, 4.5], [8, .25]])

    parser.add_argument('-main_queue_sizes', nargs='+', help='maximum sizes of the arrivals queues', default=[10, 10])
    parser.add_argument('-retry_queue_sizes', nargs='+', help='maximum sizes of the retries queues', default=[2, 2])
    parser.add_argument('-timeouts', nargs='+', help='timeouts after which the clients retry, if the jobs are not done '
                                                     '(in ms)', default=[10, 10])
    parser.add_argument('-max_retries', nargs='+', help='how many times should a client retry to send a job if it does '
                                                        'not receive a response before the timeout', default=[3, 3])
    parser.add_argument('-thread_pools', nargs='+', help='number of threads per server processing requests',
                        default=[1, 1])
    parser.add_argument('-parent_list', nargs='+',
    ...
    parser.add_argument('-q_min_list', nargs='+', help='list of minimum number of requests associated to high mode',
                        default=[int(parser.parse_args().main_queue_sizes[i] * .9) for i in
                                 range(parser.parse_args().server_num)])
    parser.add_argument('-o_min_list', nargs='+', help='list of minimum number of retrials associated to high mode',
                        default=[parser.parse_args().retry_queue_sizes[i] // 2 for i in
                                 range(parser.parse_args().server_num)])
    parser.add_argument('-q_max_list', nargs='+', help='list of maximum number of requests associated to low mode',
                        default=[int(parser.parse_args().main_queue_sizes[i] * .1) for i in
                                 range(parser.parse_args().server_num)])
    parser.add_argument('-o_max_list', nargs='+', help='list of maximum number of retrials associated to low mode',
