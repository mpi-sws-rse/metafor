...
def run_sims(max_t: float, fn: str, num_runs: int, step_time: int, sim_fn, mean_t: float, rho,
    ...
    for i in range(num_runs):
        ...
        with open(current_fn, "w") as f:
            f.write("t,rho,service_time,name,qlen,retries,dropped,runtime,retries_left,job_size \n")
...
def mean_variance_std_dev(file_names: List[str], max_t: float, num_runs: int, step_time: int,
    ...
    for l in range(num_traj):
        ...
        jlen_dataset.append(np.zeros((num_runs, num_datapoints[l])))
    run_ind = 0
            ...
            for i, line in enumerate(f.readlines()):
                ...
                if (current_cont_time < max_t) and (current_cont_time > discrete_time_point) and can_continue:
                    ...
                    for i in range(n_mid):
                        ...
                        rlen_dataset[traj_idx][run_ind, k_traj] = last_r_val
                        jlen_dataset[traj_idx][run_ind, k_traj] = last_j_val
                        k_traj += 1
                        k_overall += 1
                    last_q_val = float(split_line[4]) * 1
                    last_o_val = float(split_line[5]) * 1
                    last_r_val = float(split_line[8]) * 1
                    last_j_val = float(split_line[9]) * 1
                    qlen_dataset[traj_idx][run_ind, k_traj] = last_q_val
                    olen_dataset[traj_idx][run_ind, k_traj] = last_o_val
                    rlen_dataset[traj_idx][run_ind, k_traj] = last_r_val
                    jlen_dataset[traj_idx][run_ind, k_traj] = last_j_val
    ...
    r_seq = [[]*num_traj for l in range(num_traj)]
    j_seq = [[]*num_traj for l in range(num_traj)]
    ...
    for traj_idx in range(num_traj):
        for step in range(step_ind[traj_idx]):
            ...
            for run_ind in range(num_runs):
                ...
                j_step += jlen_dataset[traj_idx][run_ind, step] / num_runs
            ...
            r_seq[traj_idx].append(r_step)
            j_seq[traj_idx].append(j_step)
            pi_seq[traj_idx].append(pi_step)

    return q_seq, o_seq, r_seq, j_seq, pi_seq
...
def compute_mean_variance_std_deviation(fn: str, max_t: float, step_time: int, num_runs: int, mean_t: float, rho, rho_fault, fault_start, fault_duration):
    ...
    q_seq, o_seq, r_seq, j_seq, pi_seq = mean_variance_std_dev(file_names, max_t, step_time, num_runs, mean_t, rho, rho_fault, fault_start, fault_duration, 100, 20)    # fault_duration, qsize, osize
    return q_seq, o_seq, r_seq, j_seq, pi_seq
...
def run_discrete_experiment(max_t: float, runs: int, mean_t: float, rho: float, queue_size: int, retry_queue_size: int,
    ...
    if process.is_alive():
        ...
        if not done:
            q_seq, o_seq, r_seq, j_seq, pi_seq = compute_mean_variance_std_deviation(results_file_name, max_t, runs, step_time, mean_t, rho, rho_fault, fault_start, fault_duration)
    end_time = time.time()
    runtime = end_time - start_time
    print("Running time: " + str(runtime) + " s")
    q_seq, o_seq, r_seq, j_seq, pi_seq = compute_mean_variance_std_deviation(results_file_name, max_t, runs, step_time, mean_t, rho,
