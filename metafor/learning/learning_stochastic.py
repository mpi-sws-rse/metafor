...
def prepare_training_data(pi_seq, depth):
    ...
    for pi_traj in pi_seq:
        T = len(pi_traj)
        for t in range(depth, T,100):
            print(t)
            x = pi_traj[t - depth:t][0][::1]  # concatenate d history from pi_traj
            y = pi_traj[t: t + 1][0][::1]
...
def qlen_average(pi, qsize, osize) -> float:
    ...
    for n_main_queue in range(qsize):
        ...
        for n_retry_queue in range(osize):
            if index_composer(n_main_queue, n_retry_queue, qsize, osize)>=pi.shape[0]:
                continue
...
def make_non_negative(matrix):
    """Shift matrix to make all entries non-negative."""
    matrix = np.array(matrix, dtype=float)
    min_val = np.min(matrix)
    if min_val < 0:
        matrix = matrix - min_val  # Shift all values up
    return matrix

def make_stochastic(matrix, handle_negatives=True):
    """
    Convert a matrix (possibly with negatives) to stochastic.
    
    Parameters:
        matrix (list or np.ndarray): Input matrix.
        handle_negatives (bool): If True, shifts negatives to non-negative first.
    
    Returns:
        np.ndarray: Stochastic matrix (or None if invalid).
    """
    matrix = np.array(matrix, dtype=float)
    
    if handle_negatives and (matrix < 0).any():
        print("Warning: Negative entries detected. Shifting to non-negative.")
        matrix = make_non_negative(matrix)
    elif (matrix < 0).any():
        print("Error: Matrix has negative entries and cannot be stochastic.")
        return None
    
    row_sums = matrix.sum(axis=1)
    if (row_sums == 0).any():
        print("Warning: Some rows sum to 0. Replacing with uniform probabilities.")
        uniform_row = np.ones(matrix.shape[1]) / matrix.shape[1]
        matrix[row_sums == 0] = uniform_row
        row_sums = matrix.sum(axis=1)
...
theta = make_stochastic(theta)
print(theta.shape)


def is_stochastic(matrix, tol=1e-2):
    """
    Check if a matrix is stochastic (row sums = 1 and all entries ≥ 0).
    ...
    Returns:
        bool: True if stochastic, False otherwise.
    """
    matrix = np.array(matrix, dtype=float)
...
print("Is theta_red stochastic?", is_stochastic(theta_red)) 
print("Is theta stochastic?", is_stochastic(theta)) 
