...
def compare_l2_norms(model, trajectory_list):
    """
    Compare L2 norms between original states (x) and encoded latent states (z).

    Args:
        model: trained AutoEncoderModel
        trajectory_list: list of [X_traj, Y_traj] pairs (each torch tensor)
    ...
    with torch.no_grad():
        for X_traj, _ in trajectory_list:
            # X_traj shape: [T, 1, input_dim]
            X_flat = X_traj.squeeze(1)  # [T, input_dim]
            Z = model.encoder(X_flat)   # [T, latent_dim]

            x_norms.extend(torch.norm(X_flat, dim=1).cpu().numpy())
            z_norms.extend(torch.norm(Z, dim=1).cpu().numpy())
    ...
    mask = x_norms > 1e-8
    x_norms = x_norms[mask]
    z_norms = z_norms[mask]

    print(f"Max L2 norm of original space for diameter: {x_norms.max():.4f}")
    
    ratio_min = np.min(z_norms / (x_norms))
    print(f"Min ratio (latent/original): {ratio_min:.4f}")
    ratio_max = np.max(z_norms / (x_norms))
    print(f"Max ratio (latent/original): {ratio_max:.4f}")


l2_results = compare_l2_norms(model, trajectory_list)
