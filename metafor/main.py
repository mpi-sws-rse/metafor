...
def main():
    ...
    parser.add_argument(
        "-total_time",
        type=int,
        help="maximum simulation time (in s) for all the simulations",
        default=1000000,
    )
    parser.add_argument(
        "-sim_time",
    ...
    parser.add_argument(
        "-step_time", type=int, help="step time used for plots", default=500
    )
    parser.add_argument(
        "-main_queue_size",
        type=int,
        help="maximum size of the arrivals queue",
        default=100,
    )
    parser.add_argument(
        "-retry_queue_size",
        type=int,
        help="maximum size of the retries queue",
        default=20,
    )
    parser.add_argument(
        "-mean",
        type=float,
        help="mean of the exponential distribution (in ms)",
        default=0.1,
    )
    parser.add_argument(
        "-mean2",
        type=float,
        help="second mean of the bimodal distribution (in ms)",
        default=10.0,
    )
    parser.add_argument(
        "-bimod_p",
        type=float,
        help="probability of having the second mean in the bimodal " "distribution",
        default=0.001,
    )
    parser.add_argument(
        "-rho", type=float, help="server's utilization rate", default=0.8
    )
    parser.add_argument(
        "-timeouts",
        nargs="+",
        help="timeout after which the client retries, if the job is not done "
        "(in ms, can be different for different job types)",
        default=[3],
    )
    parser.add_argument(
        "-retries",
    ...
    parser.add_argument(
        "-runs",
        type=int,
        help="how many times should the simulation be run",
        default=100,
    )
    parser.add_argument(
        "-lambda0s",
        ...
        "(can be different for different job types)",
        default=[8],
    )
    parser.add_argument(
        "-mu0_ps",
        nargs="+",
        help="service rate(s) (can be different for different job types)",
        default=[10],
    )
    ...
    parser.add_argument(
        "-thread_pool",
        type=int,
        help="number of threads processing requests",
        default=1,
    )
    ...
    parser.add_argument(
        "-qlen_step",
        type=int,
        help="step size used for effect of queue length plots",
        default=10,
    )
    ...
    parser.add_argument(
        "-olen_step",
        type=int,
        help="step size used for effect of orbit length plots",
        default=10,
    )

    parser.add_argument(
        "-retry_max", type=int, help="maximum number of retries", default=10
    )
    parser.add_argument(
        "-retry_step",
        type=int,
        help="step size used for effect of retry number plots",
        default=1,
    )

    parser.add_argument(
        "-lambda_max", type=int, help="maximum arrival rate", default=12
    )
    parser.add_argument("-lambda_min", type=int, help="minimum arrival rate", default=8)
    parser.add_argument(
        "-lambda_step",
        type=int,
        help="step size used for effect of arrival rate plots",
        default=0.5,
    )
    ...
    parser.add_argument(
        "-reset_lambda_max",
        type=float,
        help="maximum arrival rate after reset",
        default=10,
    )
    parser.add_argument(
        "-reset_lambda_min",
        type=float,
        help="minimum arrival rate after reset",
        default=6,
    )
    parser.add_argument(
        "-reset_lambda_step",
        type=float,
        help="step size used for effect of arrival rate after reset",
        default=0.5,
    )
    parser.add_argument(
        "-timeout_max", type=float, help="maximum timeout after reset", default=15
    )
    parser.add_argument(
        "-timeout_min", type=float, help="minimum timeout after reset", default=5
    )
    parser.add_argument(
        "-lambda_fault",
        type=float,
        help="the arrival rate associated with the fault scenario",
        default=20,
    )
    parser.add_argument(
        "-start_time_fault",
        type=float,
        help="the time point at which the fault starts",
        default=200,
    )
    parser.add_argument(
        "-duration_fault",
        type=float,
        help="the duration of the fault event",
        default=200,
    )
